...
Environment variables:
  ...
  CF_ZONE_ID      Zone ID (find with --list-zones)
...
# cf_curl: does NOT use -f so HTTP error bodies are returned and checked via .success
cf_curl() {
  curl -sS \
...
check_response() {
  ...
  if [[ "$ok" != "true" ]]; then
    err "$context: $(echo "$resp" | jq -r '[.errors[].message] | join(", ")')"
  fi
}
...
list_zones() {
  ...
  printf "%-45s %-34s %-10s %s\n" "NAME" "ZONE ID" "STATUS" "PLAN"
  printf '%0.s─' {1..110}; echo

  while [[ $page -le $total_pages ]]; do
    local resp
    resp=$(cf_curl "$CF_API/zones?account.id=$(urlencode "$CF_ACCOUNT_ID")&per_page=50&page=$page")
    check_response "$resp" "list zones"

    total_pages=$(echo "$resp" | jq -r '.result_info.total_pages // 1')

    echo "$resp" | jq -r '.result[] | [.name, .id, .status, .plan.name] | @tsv' \
...
list_records() {
  [[ -z "$CF_ZONE_ID" ]] && err "CF_ZONE_ID is not set. Use -z <zone_id> or run --list-zones to find it."

  log "Fetching DNS records for zone $CF_ZONE_ID ..."
  local page=1 total_pages=1

  printf "%-8s %-45s %-45s %-6s %-8s %s\n" "TYPE" "NAME" "CONTENT" "TTL" "PROXIED" "ID"
  printf '%0.s─' {1..110}; echo
...
get_record_id() {
  local name="$1" type="$2"
  local resp
  resp=$(cf_curl "$CF_API/zones/$CF_ZONE_ID/dns_records?name=$(urlencode "$name")&type=$type")
...
upsert_record() {
  ...
  if [[ "$AUTO_IP" == "true" ]]; then
    ...
    RECORD_CONTENT=$(curl -sS https://api.ipify.org || curl -sS https://ifconfig.me)
  ...
  log "Done. ID: $(echo "$resp" | jq -r '.result.id') | $(echo "$resp" | jq -r '.result.name') → $(echo "$resp" | jq -r '.result.content')"
}
...
delete_record() {
  ...
  log "Deleting $record_id ($RECORD_TYPE $RECORD_NAME) ..."
  local resp
